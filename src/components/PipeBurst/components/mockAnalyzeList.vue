        ...
        <div class="affectd_users">
            <div class="table-wrapper-out">
                ...
                <div class="table-wrapper">
                    <el-table :data="effectUserInfoTemp" style="width: 100%" stripe :header-row-style="headerStyle" :row-style="rowStyle" height="100%">
...
export default {
   ...
   watch:{
       effectUserInfo(){
           this.page_total = this.effectUserInfo.length;
           this.effectUserInfoTemp = this.effectUserInfo.slice(0,this.pageSize);
       }
   },
  data() {
    let affDate = ()=>{
...
methods:{
    handleCurrentChange(val) {
        this.currentPage = val;
          this.effectUserInfoTemp = this.effectUserInfo.slice((this.currentPage - 1) * this.pageSize,this.pageSize * this.currentPage)
    },
     myEventListener(){
           var typeTemp = event.detail.type;
        if(typeTemp == "PipeBurstDraw"){
        var DownHeadMax = event.detail.data.DownHeadMax;
        this.mockData[1].value = Number(DownHeadMax).toFixed(3);
        this.mockData[2].value = Number(DownHeadMax-1).toFixed(3);
