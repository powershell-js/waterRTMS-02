...
class Methods {
    ...
    initMap(obj) { // 初始化地图,并设置中心点等
        ...
        var diffusionPositionLayer = new this.mapObj.GraphicsLayer();
        var layerDefinition = {
        ...
        map.addLayer(diffusionPositionLayer);
        ...
        var dynamicMapServiceLayerTint = new this.mapObj.ArcGISDynamicMapServiceLayer(urlClass.baseMapUrlTint);
        map.addLayer(dynamicMapServiceLayerTint); 
        dynamicMapServiceLayerTint.setVisibility(false);
        ...
        var typeMy1 = '';
         window.addEventListener('event_name',function(event){
            ...
            if(typeMy == 'DiffusionRouter'){
                console.log(typeSelectMain)
                var point = new esri.geometry.Point(typeSelectMain.X,typeSelectMain.Y,map.spatialReference);
                var typeTemp = 'Diffusion';
                isMapClickDiffusion = true;
                typeMy1 = 'DiffusionRouter';
               IdentifyMain(point,typeTemp);
            }
        })
        //告诉污染物扩散界面地图加载完了
        WindowsEvent.MyDispatchEvent("MapLoad",''); 
          /**
         * 地图监听范围改变事件
         */
        ...
        //让地图的范围发生微小变化
       /*  var extentChange = Object.assign({},global.initExtent);
        extentChange = {minX:extentChange.minX-1,minY:extentChange.minY,maxX:extentChange.maxX,maxY:extentChange.maxY};
        var initExtent = drawClass.GetMapInitExtent(extentChange)
        map.setExtent(initExtent); */
        ...
        function IdentifyMain(pointTemp,typeTemp){
            ...
            if(typeMy1 == 'DiffusionRouter')
            {
                idenrifyParams.tolerance = 100000;
                //PositionSupplyPathNode(pointTemp,diffusionLayer);
                idenrifyParams.mapExtent = initExtent;
            }
            else
            {
                idenrifyParams.tolerance = 2;
            ...
            else if(typeTemp=="SupplyPath"||typeTemp=="Diffusion"|| typeTemp == "PipeBurst")
            {
                ...
                {
                    eventType="Diffusion"
                    PositionSupplyPathNode(pointTemp,diffusionPositionLayer);
                }
        ...
        function SupplyPathResultFunction(results){
           console.log(results)
            ...
            if(isMapClickDiffusion)
            {
                ...
                if(elementIdTempMy!=elemetidTemp)
                WindowsEvent.MyDispatchEvent('DiffusionMapClickReturn',{ElementId:elemetidTemp,Label:elemetidTemp,X:pointTempMy.x,Y:pointTempMy.y});
                else
                WindowsEvent.MyDispatchEvent('DiffusionMapClickReturn',"-1");

                elementIdTempMy=elemetidTemp;
        ...
        window.addEventListener('event_name',function(event){
                ...
                else if(typeMy=="DeleteSourceIds")//删除污染物扩散的水质点
                {
                    diffusionLayer.clear();
                    diffusionPositionLayer.clear();
                    eventType = "Diffusion";
                    var positionData = event.detail.data;
                    for(var i=0;i<positionData.length;i++)
                    {
                        var obj = positionData[i];
                        var pointMy = new esri.geometry.Point(obj.X,obj.Y,map.spatialReference);
                        PositionSupplyPathNode(pointMy,diffusionPositionLayer);
                    }
                }
                ...
                {
                    dynamicMapServiceLayerTint.setVisibility(true);
                    ...
                    if(waterQualityPoint.length!=0)
                        drawClass.DrawAccuracyWaterQualityMonitor(waterQualityPoint,diffusionLayer,map,mapObjMy,dialog);
                }
                ...
                else if(typeMy=="DrawAccuracyMonitor")
                {
                    ...
                    var limitFlow = drawClass.DrawPipeFlowChange(drawData.min, drawData.max, drawData.allData,
                         global.diffusionLevel, global.pressureChangColor, pressureDisplays, flowLayer,
                        isCompare, selectTypeMain, "PlanTempCalculate", true, true);
                    ...
                    var limitPressure = drawClass.DrawPressureChange(drawData.minP, drawData.maxP, drawData.NodePressure, global.diffusionLevel, global.pressureChangColor, flowDisplays, pipeBurstNodeLayer,
                        ...
                        var planTempCalVelocityLimitTemp = drawClass.DrawPipeFlowChange(drawData.minV, drawData.maxV, drawData.strengthData, 
                            ...
                            isCompare, selectTypeMain, "PlanTempCalculate", true, true);
                        planTempCalVelocityLimit = planTempCalVelocityLimitTemp.slice(0)
                    legend.LegendMainPlanTempCalPipe(limitFlow,planTempCalVelocityLimit,tableNameMain,true);
