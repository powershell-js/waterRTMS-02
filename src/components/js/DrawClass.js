...
class Methods {
    ...
    DrawPointGraphic(width, color, point,attribute=null) {
     ...
     /*    var symbolPressureBackground = new esri.symbol.SimpleMarkerSymbol().setStyle(
            esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE).setColor(
                new esri.Color(color)).setOutline(
                    new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                         new esri.Color([233, 86, 6, 0]), 1)).setSize(width);
        var gra1 = new esri.Graphic(point, symbolPressureBackground,attribute);
        return gra1; */
    }
   ...
   */
    ...
    DrawOneTriangle(layer,pathsTemp,flow,size,color){
        ...
        var gra1 = this.CreateSVGGraphic(PictureProperty.SVGArrow, angle, new esri.geometry.Point(Number(midX), Number(midY), new esri.SpatialReference({ wkid: global.spatialReference })));
        layer.add(gra1);
    }
}
