...
class Methods {
    loadArcgis() {// 该方法用于加载 arcgis 依赖的js,css 等
        esriLoader.loadScript({ // 加载js
            //url: 'http://' + location.host + '/static/dojo/dojo.js',
             ...
             //url:'https://10.200.6.71:8084/ztdata/library/3.25/3.25/init.js',
             url:'http://10.200.6.71:8084/ztdata/library/3.25/3.25/init.js',
            dojoConfig: {
                async: true
            },
        });
        ...
        esriLoader.loadCss('http://10.200.6.71:8084/ztdata/library/3.25/3.25/esri/css/esri.css');
        esriLoader.loadCss('http://10.200.6.71:8084/ztdata/library/3.25/3.25/dijit/themes/tundra/tundra.css');
        // 加载模块
    ...
    initMap(obj) { // 初始化地图,并设置中心点等
        ...
        var supplyPathPositionLayer = new this.mapObj.GraphicsLayer();
        var layerDefinition = {
        ...
        map.addLayer(supplyPathPositionLayer);
        ...
        //var reliefMap = new this.mapObj.ArcGISDynamicMapServiceLayer(urlClass.reliefMap);
        //map.addLayer(reliefMap); 
        var dynamicMapServiceTintLayer = new this.mapObj.ArcGISDynamicMapServiceLayer(urlClass.baseMapUrlTint);
        map.addLayer(dynamicMapServiceTintLayer); 
        dynamicMapServiceTintLayer.setVisibility(false);
        ...
        function IdentifyMain(pointTemp,typeTemp){
            ...
            else if(typeTemp=="SupplyPath"||typeTemp=="Diffusion")
            {
            ...
            for(var i=0;i<attributes.length;i++)
            {
                ...
                if(obj.type=="attribute")
                {
                    ...
                    if(attributes[i].displayType=="label")
                    {
                        ...
                        attributes[i].originalData = attribute;
                ...
                {
                    var temp = CreateLabel(attributes[i].unit,attributes,i);
                    rightboxtotal[i].appendChild(temp)
                }
            } 
...
/* function GetAttribute(attributesTemp,featureTemp,resultTemp,type){
    ...
    return content;
        ...
        map.on("extent-change",function(e){
            if(myEventType=="updateTime"||myEventType=="changeLayerMy"||selectTypeMain=="SupplyAreaCompare")
            {
        ...
        function LegendDisplayMy(typeMy,timeMy){
            if(selectTypeMain=="Direction")
        ...
        window.addEventListener('event_name',function(event){
                ...
                if(typeSelectMain=="updateTime"||typeSelectMain=="changeLayerMy")
                {
                    ...
                    endDate = event.detail.endDate;
                     ...
                     ||selectTypeMain=="PressureChange"||selectTypeMain=="ValveUpdate"||selectTypeMain=="PipeStatus"||selectTypeMain=="Direction"
                     )
                    {
                        dynamicMapServiceTintLayer.setVisibility(true);
                    }
                    else
                    {
                        dynamicMapServiceTintLayer.setVisibility(false);
                    ...
                    {
                        console.log("SelectChange专题图切换")
                        console.log(isDisplays)
                ...
                else if(typeMy=="CompareWindowClose")
                {
                    ...
                    else
                    {
                        ...
                        console.log("SelectChange对比窗口关闭")
                ...
                else if(typeMy=="TimeCompareSupplyArea")
                {
                    ...
                    var extentTemp = drawClass.GetDrawMapExtent(map)//GetDrawMapExtent();
                    console.log("SelectChange供水路径对比")
                    SelectChange(selectTypeMain,timeMyMain,extentTemp);
                    LegendDisplayMain(selectTypeMain,timeMyMain)
                }
                ...
                else if(typeMy=="legendItemDisplay")
                {
                   ...
                   gl.clear();
                   if((!isCompare||selectTypeMain=="SupplyAreaCompare")&&tableNameMain==""&&EventTypeMain!="Diffusion")
                   {
                        var extentTemp = drawClass.GetDrawMapExtent(map)//GetDrawMapExtent();
                        console.log("SelectChange图例显示切换")
                        SelectChange(selectTypeMain,timeMyMain,extentTemp);
                   }
                   else if(isCompare)
                ...
                else if(typeMy=="legendAllDisplay")
                {
                    ...
                    if(isDisplay)
                    {
                        if(!isCompare||selectTypeMain=="SupplyAreaCompare")
                        {
                            ...
                            var extentTemp = drawClass.GetDrawMapExtent(map)//GetDrawMapExtent();
                            console.log("SelectChange图例全部显示切换")
                            SelectChange(selectTypeMain,timeMyMain,extentTemp);
                        }
                        else
                        GetTimeCompareData(compareData.time1,compareData.time2,compareData.min,compareData.max,compareData.field,compareData.level);
                ...
                else if(typeMy=="legendLocation")
                {
                    ...
                    var supplyAreaLabel = '';
                    if(selectTypeMain=="SupplyArea"||selectTypeMain=="PipeAge"||selectTypeMain=="SupplyAreaCompare")
                ...
                else if(typeMy=="SupplyPathCharPosition"){
                    supplyPathPositionLayer.clear();
                    var point = event.detail.data;
                    console.log(point)
                    var pointNew = new esri.geometry.Point(point.X, point.Y, new esri.SpatialReference({ wkid: global.spatialReference })); 
                    var gra1 = drawClass.CreatePictureGraphic(PictureProperty.bluePin,pointNew);
                    supplyPathPositionLayer.add(gra1)
                    map.centerAt(pointNew);
        ...
        function ValveAttributeFunction(){
            ...
            .then(function(response){
                DrawValveAttributes(response.data);
            });
        }

        function DrawValveAttributes(allValveAtttibute){
            valveAttributeLayer.clear();
        ...
        function LegendDisplayMain(typeTemp,timeTemp){
            ...
            if(typeTemp=="Flow"||typeTemp=="Velocity"||typeTemp=="Slop"||typeTemp=="Age"||typeTemp=="SupplyArea"||typeTemp=="SupplyAreaCompare")
            {
                ...
                {
                    GetBaseMax(layerType,timeMyMain,selectTypeMain,"NAN",typeMain,fieldMain);
                }
                else
                {
        ...
        */
        ...
        function PositionSupplyPathNode(geometry,layerTemp){
            ...
            layerTemp.clear();
            var point = esri.geometry.Point(geometry.x,geometry.y+20,new esri.SpatialReference({ wkid: global.spatialReference })); 
            var gra1 = drawClass.CreatePictureGraphic(PictureProperty.redPin,geometry)//new esri.Graphic(geometry, LocationMarkerSymbol);
            layerTemp.add(gra1);
        }
        ...
        function DrawPipe(re){
            ...
            else
            {
                ...
                drawClassFlowDirection.DrawPipeFlowDirection(re, gl,map, 'Result_Flow_'+timeMyMain, conditional);
            }
   ...
   /*       function DrawPipePhysicalMain(re,limits,colors,widths,typeTemp,isDisplaysTemp){
        ...
        function GetLegendDisplayData(legendPipeLengthData,type){
            ...
            for(var i=0;i<legendPipeLengthData.length;i++)
            {
                ...
                legendItem.isDisplay = true;//isDisplays[i];
