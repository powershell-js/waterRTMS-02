...
<div class="mainClass">
        <el-row class="myDivClassRow">
                ...
                <base-select :selections="list" :initLabel="initLabel" key="typeSelection" class="list" :selectValue="showObject" :nowIndex="showIndex" v-on:change="showProject"></base-select>
        ...
        <el-row style="height:100%;padding-top:40px">
            ...
            <el-button class="legendStyle" style="right:30px" v-show="!this.legendIsShow" v-on:click="legendMin">图例</el-button>
...
export default {
  ...
  mounted() {
    ...
    this.cHeight = document.documentElement.clientHeight-40;
    var that = this;
    window.onresize = function() {
      that.cWidth = document.documentElement.clientWidth;
      that.cHeight = document.documentElement.clientHeight-40;
    };
    this.initLabel = this.list[0].label;
  ...
  methods: {
    ...
    myEventListener: function(event) {
      ...
      if (typeMy == "sure") {
        ...
        if (this.showObject.value != "SupplyArea"&&this.showObject.value != "Direction") {
          var field = "Result_" + this.showObject.value + "_";
          this.GetTimeComparePipeMax(time1Temp, time2Temp, field);
        }else if(this.showObject.value == "Direction"){
          var paratem = {Time1:time1Temp,Time2:time2Temp};
           axios
            .post(
              urlClass.axiosUrl+"GetDirectionChangeData",
              JSON.stringify(paratem),
              { headers: { "Content-Type": "application/json;" } }
            )
            .then(function(response) {
              console.log(response);
              var paratem = {Time1:time1Temp,Time2:time2Temp,data:response.data};
              WindowsEvent.MyDispatchEvent('GetDirectionChangeDataReturn',paratem);
            });
        }
         else if(this.showObject.value == "SupplyArea") {
          var myEventTemp = new CustomEvent("event_name", {
            detail: { type: "This is title!" }
          });
      ...
      else if(typeMy=="legendMin"){
        this.legendMin();
      }
      else if(typeMy=="RunningStateMapLoad"){
        if(typeof(this.routerIndex)!='undefined'&&this.routerIndex!=''){
          this.showIndex = this.routerIndex;
          this.showObject = { label: "水龄", value: "Age", index: 9 };
          this.initLabel = this.list[this.showIndex].label;
          this.dispatchEventMy("changeLayerMy");
        }
      }
      else if (typeMy == "DrawFlowDirection") {
        var dataTemp = event.detail.data; 
        this.canvasapi(dataTemp);
      }
      else if(typeMy == "BaseMap"){
        this.clearFlowDirection();
      }
    },
    canvasapi:function(directionsData) {
      ...
      var draw = function() {
        ...
        for (var i = 0; i < directionsData.length; i++) {
          self.context.moveTo(directionsData[i].ntx, directionsData[i].nty);
          self.context.lineTo(directionsData[i].nfx, directionsData[i].nfy);
    ...
    PlanTempCalculateGetResult: function(response) {
      ...
      WindowsEvent.MyDispatchEvent('CalPosition','');
    },
    ...
    showProject: function() {
      ...
      if (
        ...
        if (
          ...
          this.showObject.value != "PipeStatus"&&this.showObject.value != "Age"//&&this.showObject.value != "Direction"
        ) {
