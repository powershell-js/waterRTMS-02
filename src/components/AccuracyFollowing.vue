<template>
  <div class="mainDivClass1" :style="{height:minHeight+'px'}">
    <div class="leftClassBackground" :style="{height:minHeightLeft+'px'}">
      <div class="ksfxbox">
               <div class="line"></div>
                <div class="linerighttext">在线水力模型精度分析</div>
      </div>
          ...
          <el-tabs v-model="activeName" class="tableHeaderClass" @tab-click="handleClick">
                    <el-tab-pane label="压力监测点精度" class="tablePaneClass" name="first">
                      <el-table  style="width:100%" ref="grid" :height="myHeight+'px'" :data="pressureTableData">
                        <el-table-column   prop="Name" label="监测点" :align="'center'"> 
                          ...
                          <el-table-column sortable prop="MonitorValue" label="实测值"  :align="'center'"></el-table-column>
                          <el-table-column sortable prop="CalValue" label="模拟值"  :align="'center'"></el-table-column>
                          <el-table-column sortable prop="DelValue" label="误差"  :align="'center'"></el-table-column>
                    ...
                    <el-tab-pane label="流量监测点精度" class="tablePaneClass" name="second">
                       <el-table v-if="secondIsDisplay"   style="width:100%" :height="myHeight+'px'"  ref="grid" :data="demandTableData">
                          ...
                          <el-table-column sortable prop="CalValue" label="模拟值"  :align="'center'"></el-table-column>
                          <el-table-column sortable prop="DelValue" label="误差"  :align="'center'"></el-table-column>
                          <el-table-column sortable prop="Percent" label="占比"  :align="'center'"></el-table-column>
                          <el-table-column sortable prop="Accuracy" label="精度"  :align="'center'"></el-table-column>
                          <el-table-column sortable prop="Status" label="状态"  :align="'center'"></el-table-column>
                      </el-table>
                    </el-tab-pane>
            </el-tabs>
...
import MytdtMap from "@/components/mytdt/accuracyFollowingMap";
...
export default {
  ...
  mounted() {
    ...
    this.interval = window.setInterval(this.Playing, 1000);
    var that = this;
  ...
  methods: {
    Playing(){
      ...
      if(date.getMinutes()%15==0&&date.getSeconds()==0)
      {
        ...
        dateNew.setFullYear(date.getFullYear());
        dateNew.setMonth(date.getMonth());
        dateNew.setDate(date.getDate());
        dateNew.setHours(date.getHours());
        dateNew.setMinutes(parseInt(date.getMinutes()/15*15));
        this.dateValue = dateNew;
      ...
      axios
        ...
        .then(function(respone) {
          ...
          for (var i = 0; i < self.pressureTableData.length; i++) {
            ...
            if (objTemp.Level == "-1") colorTemp = "#c4a309";
